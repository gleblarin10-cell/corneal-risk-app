...
import {
  ...
  HeartPulse, X, Sparkles, Gauge, BrainCircuit
} from "lucide-react";
...
const STORAGE_KEY = "corneal_working_app";
const historyData = [
...
const RiskBadge = ({ risk }:{ risk:number }) => (
  <span className={`badge ${risk>=65 ? "high" : risk>=35 ? "mid" : "low"}`}>{risk}%</span>
);

const Stat = ({ title, value, caption, icon:Icon }: any) => (
  <div className="stat">
...
function Login({ onLogin }:{ onLogin:(u:{email:string})=>void }){
  ...
  return (
    <div style={{minHeight:"100vh",padding:24}}>
      <div className="wrap" style={{maxWidth:1240,minHeight:"calc(100vh - 48px)",alignItems:"center"}}>
        <div className="grid g2">
          <div className="card">
            <div className="brand" style={{marginBottom:16}}>
              ...
              <div>
                ...
                <div className="muted">Clinical intelligence interface</div>
              </div>
            </div>
            <div className="grid">
              <div>
          ...
          <div className="hero">
            <div className="chip"><Sparkles size={14}/>Ophthalmology Intelligence Suite</div>
            <h1 className="title">Интеллектуальная стратификация риска для трансплантации роговицы</h1>
            <div className="sub">Современная клиническая панель с акцентом на пациента, вид вмешательства, биомаркеры воспаления и визуальную оценку риска.</div>
            <div className="grid" style={{marginTop:18}}>
              {[
                ["Patient Registry","Структурированное ведение пациентов."],
                ["Procedure Mapping","PKP, DALK, DSAEK, DMEK."],
                ["Biomarker Layer","IL-6, TNF-α, VEGF и TGF-β."],
                ["Clinical Analytics","Графики и распределение риска."]
              ].map(([t,s])=>(
                <div key={String(t)} className="softbox">
                  <div style={{fontWeight:700,color:"#fff"}}>{t}</div>
                  <div className="small muted" style={{marginTop:6}}>{s}</div>
                </div>
              ))}
            </div>
          </div>
    ...
    <div className="page">
      <aside className="side">
        <div className="brand">
          <div className="brandbox"><Eye size={24}/></div>
          <div>
            <div style={{fontWeight:800,color:"#fff",fontSize:18}}>Corneal Risk</div>
            <div className="muted small">Clinical Decision Support</div>
          </div>
        </div>
        ...
        <div className="user">
          ...
          <div className="tiny" style={{marginTop:12}}>Локальное хранение данных пациента в браузере.</div>
        </div>
      </aside>
        ...
        <div className="wrap">
          <div className="hero">
            <div className="heroGrid">
              <div>
                <div className="chip"><HeartPulse size={14}/>Corneal Graft Analytics</div>
                <h1 className="title">Интеллектуальная стратификация риска для трансплантации роговицы</h1>
                <div className="sub">Современная клиническая панель с акцентом на пациента, вид вмешательства, биомаркеры воспаления и визуальную оценку риска.</div>
              ...
              <div className="grid">
                <div className="heroStat"><div className="heroStatLabel">Пациентов в системе</div><div className="heroStatValue">{stats.total}</div></div>
                <div className="heroStat"><div className="heroStatLabel">Высокий риск</div><div className="heroStatValue">{stats.high}</div></div>
                <div className="heroStat"><div className="heroStatLabel">Средний риск</div><div className="heroStatValue">{stats.avg}%</div></div>
              </div>
            </div>
          </div>

          {page==="dashboard" && result && selected && (
            <div className="grid">
              <div className="grid g4">
                <Stat title="Всего пациентов" value={stats.total} caption="Активные записи" icon={Database}/>
                <Stat title="Высокий риск" value={stats.high} caption="Риск ≥ 65%" icon={ShieldAlert}/>
                <Stat title="Умеренный риск" value={stats.moderate} caption="Усиленный мониторинг" icon={Gauge}/>
                <Stat title="Средний риск" value={`${stats.avg}%`} caption="По всей выборке" icon={BrainCircuit}/>
              </div>

              <div className="grid g3">
                <div className="card" style={{gridColumn:"span 2"}}>
                  <h2>Сводка по пациенту</h2>
                  <div className="muted small">Текущая активная карточка и интерпретация клинического риска</div>
                  <div className="grid g2" style={{marginTop:18}}>
                    <div className="grid">
                      <div className="softbox">
                        <div style={{display:"flex",justifyContent:"space-between",gap:12}}>
                          <div>
                            <div className="muted small">Пациент</div>
                            <div style={{fontSize:24,fontWeight:800,marginTop:6,color:"#fff"}}>{selected.fullName}</div>
                            <div className="muted small">{selected.id} · {selected.age} лет · визит {selected.lastVisit}</div>
                          </div>
                          <div className="brandbox"><UserRound size={20}/></div>
                        </div>
                      </div>

                      <div className="grid g2">
                        <div className="whitebox"><div className="muted small">Тип трансплантации</div><div style={{marginTop:6,fontWeight:700,color:"#fff"}}>{labelize(selected.graftType,{primary:"Первичная",repeat:"Повторная"})}</div></div>
                        <div className="whitebox"><div className="muted small">Вид операции</div><div style={{marginTop:6,fontWeight:700,color:"#fff"}}>{selected.surgeryType}</div></div>
                        <div className="whitebox"><div className="muted small">Неоваскуляризация</div><div style={{marginTop:6,fontWeight:700,color:"#fff"}}>{labelize(selected.neovascularization,{none:"Нет",moderate:"Умеренная",severe:"Выраженная"})}</div></div>
                        <div className="whitebox"><div className="muted small">IL-6</div><div style={{marginTop:6,fontWeight:700,color:"#fff"}}>{selected.il6}</div></div>
                      </div>
                    </div>
                    ...
                    <div className="grid">
                      <div className="softbox">
                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}} className="small">
                          <span>Оценка риска</span><strong>{result.risk}%</strong>
                        </div>
                        <div className="progress"><div className="bar" style={{width:`${result.risk}%`}}/></div>
                      </div>
                      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                        <span className="badge dark">{result.level} риск</span>
                        <span className="badge outlineBadge">{result.status}</span>
                      </div>
                ...
                <div className="card">
                  <h2>Clinical Highlights</h2>
                  <div className="muted small">Ключевые параметры активной карточки</div>
                  <div className="grid" style={{marginTop:18}}>
                    {[["TNF-α",selected.tnf],["VEGF",selected.vegf],["TGF-β",selected.tgfb]].map(([t,v])=>(
                      <div key={String(t)} className="softbox">
                        <div className="muted small">{t}</div>
                        <div style={{fontSize:28,fontWeight:800,color:"#fff",marginTop:6}}>{v}</div>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {page==="patients" && selected && result && (
            <div className="grid g3">
              <div className="card">
                <h2>Пациенты</h2>
                <div className="muted small">Поиск и выбор карточки пациента</div>
                ...
                <div className="list" style={{marginTop:18}}>
                  {filtered.map(p=>{
                    const r = calc(p);
                    return (
                      <button key={p.id} className={`patient ${selectedId===p.id ? "active" : ""}`} onClick={()=>setSelectedId(p.id)}>
              ...
              <div className="card" style={{gridColumn:"span 2"}}>
                <div style={{display:"flex",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
                  <div>
                    ...
                    <div className="muted small">Подробные клинические и лабораторные сведения</div>
                  </div>
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    <button className="btn outline" onClick={()=>{ setForm(selected); setModal(true); }}>Редактировать</button>
                    <button className="btn" style={{background:"linear-gradient(135deg,#dc2626,#b91c1c)"}} onClick={()=>remove(selected.id)}>
                ...
                {tab==="summary" && (
                  <div className="grid g3" style={{marginTop:18}}>
                    {[
                      ...
                      <div key={String(l)} className="softbox">
                        <div className="muted small">{l}</div>
                        <div style={{marginTop:6,fontWeight:700,color:"#fff"}}>{v}</div>
                      </div>
                    ))}
                    <div className="whitebox" style={{gridColumn:"1 / -1"}}>
                      <div className="muted small">Заметки врача</div>
                      <div className="small" style={{marginTop:8,color:"#fff"}}>{selected.notes || "—"}</div>
                    </div>
                  </div>
                )}
                ...
                {tab==="markers" && (
                  <div className="grid g2" style={{marginTop:18}}>
                    {[["IL-6",selected.il6],["TNF-α",selected.tnf],["VEGF",selected.vegf],["TGF-β",selected.tgfb]].map(([l,v])=>(
                      <div key={String(l)} className="softbox">
                        <div className="muted small">{l}</div>
                        <div style={{fontSize:22,fontWeight:800,marginTop:6,color:"#fff"}}>{v}</div>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {tab==="recommendations" && (
                  <div className="grid" style={{marginTop:18}}>
                    <div className="softbox">
                      ...
                      <div style={{fontSize:30,fontWeight:800,marginTop:6,color:"#fff"}}>{result.risk}%</div>
                      <div className="muted small" style={{marginTop:6}}>{result.level} риск · {result.status}</div>
                    </div>
                    <div className="whitebox">
                      <div className="muted small">Рекомендация</div>
                      <div className="small" style={{marginTop:8,color:"#fff"}}>{result.recommendation}</div>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="grid">
              <div className="grid g2">
                ...
                <div className="card">
                  ...
                  <div className="muted small">По всей текущей выборке пациентов</div>
                  <div style={{height:320,marginTop:10}}>
                    <ResponsiveContainer width="100%" height="100%">
              ...
              <div className="card">
                ...
                <div className="muted small">Пример временного ряда для клинического наблюдения</div>
          ...
          {page==="model" && (
            <div className="grid g3">
              <div className="card" style={{gridColumn:"span 2"}}>
                <h2>Логика модели риска</h2>
                <div className="muted small">Как система может работать в реальной клинической среде</div>
                <div className="grid" style={{marginTop:18}}>
                  {[
                    ["Входные признаки","Тип трансплантации, вид операции, неоваскуляризация, воспаление, эпизоды отторжения, IL-6, TNF-α, VEGF и TGF-β."],
                    ["Возможные алгоритмы","Логистическая регрессия, Random Forest, XGBoost и ансамблевые модели."],
                    ["Выход модели","Вероятность риска отторжения и стратификация пациента по группам наблюдения."],
                    ["Метрики качества","ROC-AUC, sensitivity, specificity, F1-score, calibration curve и внешняя валидация."]
                  ].map(([t,s])=>(
                    <div key={String(t)} className="softbox">
                      <div style={{fontWeight:700,color:"#fff"}}>{t}</div>
                      <div className="small muted" style={{marginTop:6}}>{s}</div>
              ...
              <div className="card">
                ...
                <div className="grid small muted" style={{marginTop:18}}>
                  <div>• Малые клинические выборки</div>
                  <div>• Риск переобучения моделей</div>
                  <div>• Неполные данные</div>
                  <div>• Необходимость внешней валидации</div>
                  <div>• Требование интерпретируемости для врача</div>
                </div>
              </div>
            </div>
          )}
          ...
          {page==="about" && (
            <div className="grid g2">
              <div className="card">
                <h2>О платформе</h2>
                <div className="grid small muted" style={{marginTop:18}}>
                  <div>Платформа объединяет клинические данные, вид операции, биомаркеры и аналитические визуализации в едином интерфейсе.</div>
                  <div>Система может использоваться как цифровой контур поддержки принятия решений в офтальмологической практике.</div>
                  <div>Архитектура готова для дальнейшего подключения полноценной прогнозной модели и серверной части.</div>
                </div>
              </div>

              <div className="card">
                <h2>Следующие шаги развития</h2>
                <div className="grid small muted" style={{marginTop:18}}>
                  <div>• Подключить PostgreSQL</div>
                  <div>• Добавить роли пользователей</div>
                  <div>• Реализовать REST API</div>
                  <div>• Обучить модель на реальном датасете</div>
                  <div>• Добавить интеграцию с ЭМК</div>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="modal">
            <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"start"}}>
              <div>
                ...
                <div className="muted small">Заполните клинические и лабораторные параметры для расчёта риска.</div>
              </div>
